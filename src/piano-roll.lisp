  ...
  (let* ((stream (find-pane-named *application-frame* 'piano-roll-pane))
    ...
    (block tracking-pointer
      (tracking-pointer (stream)
        (:pointer-motion (x)
                         (let* ((modifier-state (port-modifier-state (find-port)))
                                (shift-p (or (logtest modifier-state +shift-key+)
                                             (logtest modifier-state +control-key+)))
                                (x-beat (funcall (if shift-p #'x-pixel-to-beat #'x-pixel-to-beat-quantized)
                                                 x *application-frame*))
                                (grid-size (grid-size *application-frame*))
                                (new-sustain (max grid-size (- x-beat event-beat)))
